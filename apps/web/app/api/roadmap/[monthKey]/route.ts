...
export async function GET(_: Request, { params }: { params: { monthKey: string } }) {
  const ws = getNodeTargetsFromEnv()[0];
  try {
    const res = await fetch(`${ws.url}/roadmap/${params.monthKey}`, { cache: "no-store" });
...
export async function POST(req: Request, { params }: { params: { monthKey: string } }) {
  ...
  try {
    const res = await fetch(`${ws.url}/roadmap/${params.monthKey}`, {
      method: "POST",
